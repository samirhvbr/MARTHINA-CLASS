...
#
# Deploy idempotente do Marthina Learning (Laravel 12 / PHP 8.2+) em
# /srv/www/marthina.com.br. Web server: Nginx + PHP-FPM. Banco: MariaDB/MySQL.
# Modelado no deploy.sh do ONLINE/INTRANET (mesma frota).
#
...
# - Pula composer quando composer.lock não mudou; pula npm install quando as
#   dependências não mudaram — mas o BUILD roda SEMPRE (Tailwind 4 reescaneia as
#   fontes blade/js a cada build; uma classe nova numa view exige rebuild).
...
#   bash /srv/www/marthina.com.br/deploy.sh
...
# ── Config ────────────────────────────────────────────────────────────────────
DIR="/srv/www/marthina.com.br"
LOCK="${DIR}/.deploy.lock"
BACKUP_DIR="/var/backups/marthina"
...
cleanup_on_failure() {
    ...
    notify "❌ <b>Deploy Marthina falhou</b> em <code>$(as_app git rev-parse --short HEAD 2>/dev/null || echo '?')</code>"
}
...
log "=== Deploy Marthina — dir: $DIR | app: $APP_USER | web: $WEB_USER | branch: $BRANCH ==="
...
if [ "$BACKUP" = "1" ]; then
    if [ -n "$DB_NAME" ] && [ -n "$DB_USER" ]; then
        mkdir -p "$BACKUP_DIR"
        BACKUP_FILE="$BACKUP_DIR/marthina_$(date +%Y%m%d_%H%M%S).sql.gz"
        if MYSQL_PWD="$DB_PASS" mysqldump \
            ...
            ls -t "$BACKUP_DIR"/marthina_*.sql.gz 2>/dev/null | tail -n +11 | xargs -r rm --
        else
...
# ── 6. Frontend (npm install só se deps mudaram; build SEMPRE — Tailwind) ─────
log "==> [6/9] Frontend (build sempre — Tailwind reescaneia as fontes)..."
...
log "✅ Deploy Marthina concluído: $SHORT_SHA — $SUBJECT"
notify "✅ <b>Deploy Marthina concluído</b>: <code>$SHORT_SHA</code> — $SUBJECT"
